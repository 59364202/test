...
//			Param_Provinces
//	Return:
//		Array Struct_Pressure
func GetPressure(p *Param_Provinces) ([]*Struct_Pressure, error) {
	...
	if p.Order != "" {
		q += " ORDER BY pressure_value " + p.Order
	}
	...
	for row.Next() {
		var (
			...
			_pressure_datetime    sql.NullString
			_pressure_value       sql.NullFloat64

			d       *Struct_Pressure              = &Struct_Pressure{}
		...
		err = row.Scan(&_pressure_datetime, &_pressure_value, &_tele_station_id, &_tele_station_oldcode, &_name, &_tele_station_lat, &_tele_station_long, &_area_code, &_area_name, &_province_code, &_amphoe_code, &_tambon_code, &_province_name, &_amphoe_name, &_tumbon_name, &_agency_id, &_agency_name)

		if err != nil {
			return nil, err
		}

		d.Pressure = ValidData(_pressure_value.Valid, _pressure_value.Float64)
		if _pressure_datetime.Valid {
			d.PressureDatetime = pqx.NullStringToTime(_pressure_datetime).Format(strDatetimeFormat)
		}
