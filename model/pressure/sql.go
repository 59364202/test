...
var sqlGraph = `SELECT data.id,gs.datetime, pressure_value
	FROM public.pressure data
	...
	RIGHT JOIN ( SELECT generate_series ($2::date, $3, '1 hour' ) AS datetime ) gs 
		ON data.pressure_datetime between $2 AND $3
		AND data.pressure_datetime = gs.datetime  AND data.deleted_at = to_timestamp (0)  AND ( qc_status IS NULL OR qc_status ->> 'is_pass' = 'true' ) 
	ORDER BY gs.datetime ASC `

var sqlPressure = `SELECT 
        dd.pressure_datetime,
        pressure_value,
        d.id AS station_id,
  ...
  WHERE 
  pressure_datetime::date = CURRENT_DATE AND ((d.is_ignore = false) AND ((dd.qc_status IS NULL) OR ((dd.qc_status ->> 'is_pass'::text) = 'true'::text)))
`
