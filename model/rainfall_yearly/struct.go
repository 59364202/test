...
type Struct_RainfallYearly struct {
	...
	StationType   string      `json:"station_type"`      // example:`rainfall_yearly` ชนิดของสถานี
...
type Struct_TeleStation struct {
	...
	Type    string          `json:"tele_station_type"`           // example:`rainfall_yearly` ชนิดของสถานี
	SubBasin_id 	int64       `json:"sub_basin_id,omitempty"`      // example:`21` รหัสลุ่มน้ำย่อย
}

type Param_RainfallYearly struct {
	IsDaily   bool
	Order     string `json:"order"`
	DataLimit int    `json:"data_limit"`
	StationId int64  `json:"station_id"`
	StratDate string `json:"start_date"`
