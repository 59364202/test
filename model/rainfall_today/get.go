...
// Package rainfall_today is a model for public.rainfall_today table. This table store rainfall_today.
...
//			Param_RainfallToday
//	Return:
//		Array Struct_RainfallToday
func GetRain(p *Param_RainfallToday) ([]*Struct_RainfallToday, error) {
	db, err := pqx.Open()
	if err != nil {
		return nil, err
	}
	...
	data := make([]*Struct_RainfallToday, 0)
	for row.Next() {
		...
		obj.Id = _data_id
		if p.IsToday {
			obj.StationType = "rainfall_today"
		} else {
			obj.StationType = "rainfall_daily"
		}
...
//	get rainfall today graph
//	Parameters:
//		p
//			Param_RainfallToday
//	Return:
//		Array Struct_RainfallToday_Graph
func GetRainGraph(p *Param_RainfallToday) ([]*Struct_RainfallToday_Graph, error) {
	if p.StationId <= 0 {
		return nil, rest.NewError(422, "invalid station_id ", nil)
	}
	...
	var (
		obj       *Struct_RainfallToday_Graph
		_date     sqltime.NullTime
		_rainfall sql.NullFloat64
	)
	data := make([]*Struct_RainfallToday_Graph, 0)
	for row.Next() {
