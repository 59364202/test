...
//	genarate sql string
func Gen_SQL_GetRainfall7d(p *Param_Rainfall7d) (string, []interface{}){
	var param []interface{}
	var (
		strDayInterval int = 7 // ย้อนหลัง 7 วัน
		strSql        string
		strWhere      string 
	)
	...
	WITH sum_query AS (
		SELECT 
			tele_station_id,sum( rainfall_value) AS rainfall7d,ROW_NUMBER () OVER (PARTITION BY province_code ORDER BY sum( rainfall_value) DESC) AS ROW
		...
		ORDER BY province_code,rainfall7d DESC
	)
	SELECT 
		tele_station_id, date_trunc('day'::text, now() - '` + strconv.Itoa(strDayInterval) + ` days'::interval) AS start_date, 
		date_trunc('day'::text, now() - '1 days'::interval) AS end_date,rainfall7d,
