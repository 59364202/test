...
func GetRainfall7d(p *Param_Rainfall7d) ([]*Struct_Rainfall7d, error){
	db, err := pqx.Open()
	if err != nil {
		return nil, err
	}

	strSql, param := Gen_SQL_GetRainfall7d(p)
	row, err := db.Query(strSql, param...)
	if err != nil {
		return nil, pqx.GetRESTError(err)
	}

	data := make([]*Struct_Rainfall7d, 0)
	...
	for row.Next() {
		...
		err = row.Scan(&_tele_station_id, &_rainfall_datestart, &_rainfall_dateend, &_rainfall7d, &_tele_station_oldcode, &_tele_station_lat, &_tele_station_long, &_area_code, &_province_code, &_area_name, &_province_name, &_amphoe_name, &_tumbon_name, &_agency_id, &_agency_name, &_name)

		if err != nil {
			return nil, err
		}

		d.Rain7D = fmt.Sprintf("%.2f", _rainfall7d.Float64)
